...
def load_dataset(dataset_name = "genius", K = 6, tau = 0.5, tau1 = 1.0, tau2 = 1.0, self_loop = True):  
    features_time  = 0
    lazy_update_time = 0
    sparse_mx_time = 0
    ...
    LP1, lazy_update_time, sparse_mx_time = edgeindex_construct(edge_index, num_nodes, tau1)    
    LP2, lazy_update_time, sparse_mx_time = edgeindex_construct(edge_index, num_nodes, tau2)    
    t1 = time.time()            
    features = [feat]
    basis = feat  
    basis1 = feat 
    ...
    for i in range(1,K+1):
        ...
        basis2 = torch.spmm(LP2, basis2)               
        features.append((basis+basis1+basis2)/3.0)
